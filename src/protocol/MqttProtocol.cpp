...
void on_message( struct mosquitto *client, void *obj,
...
MqttProtocol::~MqttProtocol() { logger::trace( this->name + ": destruct" ); }
...
void MqttProtocol::configure()
{
    ...
    // Recreation
        ...
        int res =
            mosquitto_reinitialise( client, identifier.c_str(), true, NULL );
    ...
    // Configure last will
    logger::info( this->name + ": configure topic:{} message:{}",
                  lastWill.topic.c_str(), lastWill.payload.c_str() );
    res = mosquitto_will_set( client, lastWill.topic.c_str(),
                              lastWill.payload.size(), lastWill.payload.c_str(),
                              lastWill.qos, lastWill.retained );
    if ( res != MOSQ_ERR_SUCCESS ) {
        logger::error( mosquitto_strerror( res ) );
        notify( "error", nullptr );
        return;
                 ...
                 const struct mosquitto_message *message )
{
    ...
    logger::info( p->getName() + ": received message: topic:{} payload:{}",
                  topic, payload );
}
...
void on_subscribe( struct mosquitto *client, void *obj, int mid, int qos_count,
                   const int *qos_grand )
{
    MqttProtocol *p = (MqttProtocol *)obj;
